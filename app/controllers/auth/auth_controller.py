...
@auth.route('/register' , methods=['POST'])#post is used 
def register_author():
...
#Validations of the incoming request.
    ...
    if len(password) < 8:#password has to be less then eight letters
    ...
    try:
        hashed_password = bcrypt.generate_password_hash(password)#we are hashing the password wc is uused to encrypt
...
@auth.route("token/refresh", methods=["POST"])##POST IS USED TO CREATE ANYTHING
